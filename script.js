const API_URL = 'https://shortcake-opossum-splashy.ngrok-free.dev';
...
document.addEventListener('DOMContentLoaded', () => {
                    ...
                    if (step.matrix) {
                        ...
                        step.matrix.forEach((r, rIdx) => {
                            ...
                            r.forEach((val, cIdx) => {
                                ...
                                const isCoveredRow = step.row_lines && step.row_lines[rIdx];
                                const isCoveredCol = step.col_lines && step.col_lines[cIdx];
                                
                                let bgStyle = isZero ? 'background: #FEF2F2;' : 'background: transparent;';
                                let borderStyle = '';
                                
                                if (isCoveredRow && isCoveredCol) {
                                    bgStyle = 'background: repeating-linear-gradient(45deg, #E2E8F0, #E2E8F0 10px, #F1F5F9 10px, #F1F5F9 20px);';
                                    borderStyle = 'outline: 2px solid #94A3B8; outline-offset: -2px;';
                                } else if (isCoveredRow) {
                                    bgStyle = 'background: rgba(148, 163, 184, 0.15);';
                                    borderStyle = 'border-top: 2px solid #64748B; border-bottom: 2px solid #64748B;';
                                } else if (isCoveredCol) {
                                    bgStyle = 'background: rgba(148, 163, 184, 0.15);';
                                    borderStyle = 'border-left: 2px solid #64748B; border-right: 2px solid #64748B;';
                                }
                                
                                stepsHtml += `<td style="padding: 0.4rem 0.6rem; ${isZero ? 'color: #EF4444; font-weight: bold;' : 'color: #334155;'} ${bgStyle} ${borderStyle}">${Math.round(val)}</td>`;
                            });
                            stepsHtml += '</tr>';
                        });
                        stepsHtml += '</table></div>';
    ...
    async function solveCpm() {
        ...
        // Validasi Predecessor (Scenario 19)
        let invalidPred = null;
        Object.keys(activities).forEach(id => {
            const predsRaw = activities[id].predecessors;
        ...
        if (invalidPred) {
            alert(`Gagal: ID Predecessor "${invalidPred}" belum didaftarkan. Harap masukkan ID yang valid.`);
            return;
        }
